...
function CreateEvent () {
  ...
  async function handleSubmit (e) {
    ...
    try {
      const res = await axios.post('http://localhost:5000/events/create_event', {...formData, organized_by: userId}, {
  ...
  const validateTime = () => {
    ...
    if (start_time) {
      ...
      if (eventDate.toDateString() === now.toDateString() && startDateTime < now) {
      ...
      <div className="max-w-6xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="bg-white bg-opacity-80 backdrop-blur-lg rounded-3xl shadow-2xl overflow-hidden">
          <div className="relative h-48 sm:h-64 md:h-80 overflow-hidden">
            {imageUploadError && <span className='text-xs text-red-600 flex justify-center items-center gap-1'><MdError />{imageUploadError}</span>}
            {isUploading ? (
              <div className="absolute inset-0 flex items-center justify-center bg-gray-200 bg-opacity-75">
                <AiOutlineLoading3Quarters className="w-12 h-12 text-purple-600 animate-spin" />
              </div>
            ): formData.image_url ? (
              <button type='button' className='w-full h-full' onClick={() => imageRef.current.click()}><img src={formData.image_url} alt="Event header" className="w-full h-full object-cover" /></button>
            ) : (
                ...
                <div>
                  ...
                  <div className="mt-1 relative rounded-md shadow-sm">
                    <input
                      ...
                      required
                      className={`block w-full border-2 ${dateError ? 'border-red-300' : 'border-gray-300'} rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent`}
                      />
                  </div>
                  {dateError && <span className='text-sm text-red-600 flex items-center gap-1 p-1'><MdError />{dateError}</span>}
                </div>
                ...
                <div>
                  ...
                  <div className="mt-1 relative rounded-md shadow-sm">
                    <input
                      ...
                      onChange={handleChange}
                      required
                      className={`text-purple-600 block w-full border-2 ${startTimeError ? 'border-red-300' : 'border-gray-300'} rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent`}
                    />
                  </div>
                  {startTimeError && <span className='text-sm text-red-600 flex items-center gap-1 p-1'><MdError />{startTimeError}</span>}
                </div>
                ...
                <div>
                  ...
                  <div className="mt-1 relative rounded-md shadow-sm">
                    <input
                      ...
                      onChange={handleChange}
                      required
                      className={`text-purple-600 block w-full border-2 ${endTimeError ? 'border-red-300' : 'border-gray-300'} rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent`}
                      />
                  </div>
                  {endTimeError && <span className='text-sm text-red-600 flex items-center gap-1 p-1'><MdError />{endTimeError}</span>}  
                </div>
